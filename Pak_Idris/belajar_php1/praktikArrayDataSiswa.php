...
<div class="container">
    ...
    <?php foreach ($nilai_kelas as $nama_siswa => $nilai_siswa): ?>
        ...
        ?>
        <div class="student-card" style="border-color: <?php echo htmlspecialchars($warna); ?>;">
            ...
            <div class="subjects">
                <?php foreach ($nilai_siswa as $mata_pelajaran => $nilai): ?>
                    ...
                    ?>
                    <div class="subject">
                        <span>📚 <?php echo htmlspecialchars(ucfirst(str_replace("_", " ", $mata_pelajaran))); ?>:</span>
                        <span class="<?php echo $kelas_grade; ?>"><?php echo htmlspecialchars($nilai); ?></span>
                    </div>
                <?php endforeach; ?>
            </div>

            <div style="margin-top: 15px; padding: 10px; background-color: white; border-radius: 5px;">
                <strong>Rata-rata: <?php echo htmlspecialchars($rata_rata); ?></strong> |
                <strong style="color: <?php echo htmlspecialchars($warna); ?>"><?php echo htmlspecialchars($status); ?></strong>
            </div>
        </div>
    <?php endforeach; ?>

    <!-- Statistik Kelas -->
    <div style="border: 2px solid #9b59b6; padding: 20px; border-radius: 10px; margin-top: 30px;">
        ...
        <?php
            ...
            if (count($semua_nilai) > 0) {
                $rata_rata_kelas = round(array_sum($semua_nilai) / count($semua_nilai), 2);
        ...
        ?>
        <p>👥 Jumlah Siswa: <strong><?php echo $jumlah_siswa; ?></strong></p>
        <p>📊 Rata-rata Kelas: <strong><?php echo htmlspecialchars($rata_rata_kelas); ?></strong></p>
        <p>🚀 Nilai Tertinggi: <strong><?php echo htmlspecialchars($nilai_tertinggi); ?></strong></p>
        <p>📉 Nilai Terendah: <strong><?php echo htmlspecialchars($nilai_terendah); ?></strong></p>
    </div>
